...
void			draw(t_env *env)
{
	...
	mlx_put_image_to_window(env->window->mlx, env->window->mlx_window
			, env->window->img, 0, 0);
	char	*str;
	str = env->scenario == RAIN ? "Rain" : (env->scenario == UPRISING ? "Uprising" : (env->scenario == WAVE ? "Wave" : (env->scenario == EMPTYING ? "Emptying" : "Escaping"))); // writes to the window the name of what is happening
	mlx_string_put(env->window->mlx, env->window->mlx_window, 11, 11, 0, str);
	mlx_string_put(env->window->mlx, env->window->mlx_window, 10, 10, 0xECECEC, str);//0xFFFFFF 0xff0000 //Changes colour of text
